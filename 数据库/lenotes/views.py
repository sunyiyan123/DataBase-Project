...
def del_member(request, group_id, info_id):
    """ 删除成员 """
    """ print('fuck!!!!!!!!!!!!!!!!' + str(group_id))
    print('fuck!!!!!!!!!!!!!!!!' + str(info_id)) """
...
def new_diary(request, group_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            ...
            new_diary.diary_log = str(request.user)
            new_diary.save()
...
def edit_diary_md(request, diary_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            diary.diary_log = str(request.user)
            form.save()
            return HttpResponseRedirect(reverse('lenotes:group', args=[group.id]))
    context = {
        'diary': diary, 
...
@login_required
def single_diary(request, diary_id):
    """编辑既有条目"""
    diary = Diary.objects.get(id=diary_id)
    group = diary.group
    check_user(request, group, 1)
    ...
    return render(request, 'lenotes/single_diary.html', context)
